...
//  .setCharacteristic(Characteristic.SecuritySystemTargetState,'3');
  ...
  this._currentStateChar.on('get', (callback) => {
      ...
      this.log.debug("current state = "+armStatus);
 ...
 /* this._currentStateChar.on('set', (value,callback) => {
      this.log.debug('set current state event handler');
    callback();
  });
*/
  this._targetStateChar.on('set', (state,callback) => {
     ...
     this.log.debug("target set "+state);
//this._service.setCharacteristic(Characteristic.SecuritySystemCurrentState,state);
    // this._service.setCharacteristic(Characteristic.SecuritySystemTargetState,state);
  }
     callback(null, state);
  ...
  this.elk.on('AS', (armStatusMsg)=> {
      ...
      this.log.debug(area);
      var armStatus = this.HKStatusFromElkStatus(area);
      this.log.debug("AS current state = "+armStatus);
      this.targetState = armStatus;
      this.currentState = armStatus;
     // this._currentStateChar.setValue(armStatus,undefined,'event');
